...
import { UserProfile, PackageTemplate, DocumentTypeItem, KodeRekening, SpjItem } from "../types";
import {
...
import { FileStack, Plus, Edit, Trash2, Save, RefreshCw, Info, X, Package, FolderOpen, ArrowRight } from "lucide-react";
...
export const PackageTemplates: React.FC<PackageTemplatesProps> = ({ user, onOpenSpj }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [tList, dList, rList, sList] = await Promise.all([
