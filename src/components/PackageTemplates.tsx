...
import { UserProfile, PackageTemplate, ChecklistConfig, DocumentTypeItem, KodeRekening, SpjItem } from "../types";
import {
...
import { FileStack, Plus, Edit, Trash2, Save, RefreshCw, Info, X, Package, FolderOpen, ArrowRight, ShieldCheck } from "lucide-react";

interface PackageTemplatesProps {
  user: UserProfile;
  /** Buka paket SPJ yang sudah ada di editor (dari menu ini juga). */
  onOpenSpj?: (spjId: string, mode: "edit" | "preview") => void;
}
...
export const PackageTemplates: React.FC<PackageTemplatesProps> = ({ user, onOpenSpj }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [tList, cList, dList, rList, sList] = await Promise.all([
        ...
        // Paket SPJ yang sudah dibuat ikut ditampilkan di menu ini agar dapat
        // langsung dibuka dan diedit tanpa berpindah menu.
        getSpjList(user).catch(() => [] as SpjItem[]),
      ]);
      setTemplates(tList);
      setConfigs(cList);
      setDocTypes(dList);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* PAKET SPJ YANG SUDAH ADA — dapat langsung dibuka & diedit dari sini */}
      {spjs.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100 dark:border-slate-700 flex items-center space-x-3">
            <div className="p-2 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl">
              <FolderOpen className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
              <h2 className="font-bold text-gray-900 dark:text-white text-sm">
                Paket SPJ yang Sudah Ada ({spjs.length})
              </h2>
              <p className="text-xs text-gray-500 dark:text-slate-400">
                Buka dan edit paket yang sudah dibuat tanpa berpindah menu
              </p>
            </div>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-slate-700">
            {spjs.map((s) => (
              <div
                key={s.id}
                className="p-4 flex flex-wrap items-center gap-3 hover:bg-gray-50/60 dark:hover:bg-slate-700/30 transition"
              >
                <div className="min-w-0 flex-1">
                  <p className="font-mono text-[11px] font-bold text-[#32848D]">{s.nomorSpj}</p>
                  <p className="text-xs font-semibold text-gray-900 dark:text-white truncate">
                    {s.masterSnapshot?.kegiatan?.nama || "—"}
                  </p>
                  <p className="text-[11px] text-gray-500 dark:text-slate-400 truncate">
                    [{s.masterSnapshot?.kodeRekening?.kode}] {s.masterSnapshot?.kodeRekening?.nama}
                  </p>
                  <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">
                    {formatDateDDMMYYYY(s.tanggal)} · {s.jawatanName}
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {onOpenSpj && (
                    <>
                      <button
                        onClick={() => onOpenSpj(s.id, "preview")}
                        className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-200 hover:bg-gray-200 dark:hover:bg-slate-600"
                      >
                        Preview
                      </button>
                      <button
                        onClick={() => onOpenSpj(s.id, "edit")}
                        className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-[#32848D] hover:bg-[#276972] text-white inline-flex items-center gap-1"
                      >
                        <span>Kelola</span>
